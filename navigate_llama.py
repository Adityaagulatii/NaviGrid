...
# ── Full route (always starts from entrance) ───────────────────────────────
FULL_ROUTE = [
    "entrance",
    "room_045",
    "room_040",
...
memory = load_memory()
print(f"✅ LLaMA memory loaded ({len(memory['history'])} past instructions)")
...
def get_llama_instruction(current, next_node, route_progress):
    ...
    if memory["history"]:
        ...
        past  += "\n".join([f"- {h['current']} → {h['next']}: {h['instruction']}"
                            for h in recent])
    key      = f"{current}→{next_node}"
    feedback = memory["feedback"].get(key, "")
...
Learn from past instructions to improve. No markdown, plain text only."""
...
def select_destination():
    destinations = [n for n in FULL_ROUTE if n != "entrance"]
    print("\n🗺️  NaviGrid - Indoor Navigation")
    print("=" * 40)
    print("📍 Starting from: Entrance (default)")
    print("\n🎯 Select your destination:")
    for i, node in enumerate(destinations):
        label = node.replace("room_", "Room ").replace("_", " ").title()
        print(f"  {i+1}. {label}")
    while True:
        try:
            choice = int(input("\nEnter number: ")) - 1
...
def draw_minimap(frame, navigator):
    ...
    for node, pos in NODE_POS.items():
        ...
        if step == navigator.current_step:
            ...
            cv2.putText(minimap, node.replace("room_", "R"),
                        (pos[0]+16, pos[1]+5),
    ...
    cv2.putText(minimap, "MAP", (cx-12, mh-8),
...
def main():
    ...
    # Step 2: Build route from entrance to destination
    dest_idx = FULL_ROUTE.index(destination)
    route    = FULL_ROUTE[:dest_idx + 1]
    print(f"\n📍 Route: {' → '.join(route)}")

    # Step 3: Start camera and navigation
    ...
    while True:
        ...
        cv2.imshow("NaviGrid + LLaMA", frame)
